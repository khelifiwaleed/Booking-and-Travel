...
<!--==============================content================================-->
    <section id="content">
        ...
        <div class="wrapper">
                       ...
                       while($var = mysql_fetch_assoc($req))
                                  {
									...
									$nomhotel = $var['nom'] ;
										...
										$sql_geo = mysql_query("SELECT * FROM `geolocalisation` WHERE `select_id` = '$hotel_id' AND `type` = \"h\" LIMIT 0 , 1 ");
           ...
           <a href=\"../include/reserver_hotel.php?idnom=".$var['id'] ."\"> <font color=#ffffff ><h1> Reserver </h1></font> </a></div><br>"
       ) ;              ?>
              </div>
              <h2 class="ind3"> <?php
				                   echo $var['nom'] ;
              ...
              <div class="wrapper team_box m_bot1">
			   ...
			   ?>  
               <?php
                      $sql2 = "SELECT * FROM `image` WHERE `select_id` = '$hotel_id' && `type` = \"h\" LIMIT 0 , 3";
        ...
        <hr>
        <h3> <font color="#FF0000" > Les Commentaire sur Hotel <?php  echo $nomhotel; ?> 
                                  </font></h3>
        <br><br><br><br>
	 ...
	 if(isset($_POST['submit']))
	  {
		...
		if($nom && $email && $message ) 
		{
			...
			{
			$query = mysql_query (" INSERT INTO `db_pfe`.`livredor` (`id`, `nom`, `email`, `message`, `id_select`, `type`) VALUES (NULL, '$nom', '$email', '$message', '$id', 'h') " )  ;	
			if ($query)
			{
		...
		$select = mysql_query("SELECT * FROM livredor WHERE `id_select` = '$id' && `type` = \"h\"  ORDER BY id DESC ");
